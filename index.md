...
The current project used the Neuromatch Academy dataset for the perceptual dot motion decision-making task {cite:p}`LaquitaineSwitchingObserverHuman2018a`, where subjects were required to estimate the motion direction of random dots under two varying conditions:  1) motion coherence (percentage of dots moving in the same direction), and 2) prior standard deviation (the sharpness of the presented dot distribution). In the original paper, Bayesian observers are described to integrate the visual measurement from the sampled distribution with the prior distribution to form an estimate, whereas Switching observers will choose either sampling with the prior mean and evidence distribution depending on their relative strength. For example, when motion coherence is low, subjects tend to choose estimates from the prior distribution, leading to a bias toward estimates near the prior, whereas an increase in prior standard deviation introduces greater variability in subjects’ estimates.The switching observer model yielded a better fit than the Bayesian observer model (the posterior distribution resulting from the observed percept in a particular trial by the prior), as it could account for the bimodality distribution of individuals’ estimates when the presented motion was very different from the constant prior experimental mean of  225˚. 

We aim to examine whether switching behavior is time-dependent and whether factors such as motion coherence can influence individuals' internal states during decision-making and whether observers employ multiple strategies during decision making. Recent reviews have challenged the assumption that decision strategies are stable across trials and have shown that models that deploy non-stationary dynamics, such as discrete-state switches, provide greater explanatory power for human behaviour than some standard static models {cite:p}`GunawanTimeevolvingPsychologicalProcesses2022;uraiStructureUncoveredUnderstanding2026a`. Therefore, we compared 3 models: Probabilistic Mixture model (PM), Weighted Mean model (WM) and the Hidden Markov Model.
...
Results demonstrated a model fit of the input HMM compared with PM, $\Delta$ NLL= 4121.44, $\Delta$ AIC= 1996.22, $\Delta$ BIC= 1772.32. While previous static models correctly identified that observers switch between discrete prior-dependent and evidence-dependent states, our work demonstrates that this switching is a dynamic, memory-retaining process driven by temporal inertia and actively updated by shifting covariates, such as motion coherence and large stimulus-angle deviations(@figure-param ). Our study extends the switching observer phenomenon by providing a temporal account for individual differences in strategy switching (@figure-main). Given high prior standard deviation(where difficulty level is also high as the displayed angle was more dispersed),subject 1 shows a more frequent pattern of staying in the evidence state than subject 10. This specific pattern was also reflected in subject-wise accuracy rate, where subject 1 has a mean error rate of 29.38% and subject 10 has 67.17% error, respectively, implying these fluctuations discrepancies could reflect attentional allocation and that relying on priors with changing coherence might not be optimal.
